...
let is_valid passphrase =
  ...
  let unique = StringSet.of_list words in
  List.length words = StringSet.cardinal unique
;;

let is_more_valid passphrase =
  let words = String.split_on_char ' ' passphrase in
  let unique =
    words
    |> List.map (fun s ->
...
let run () =
  input |> Util.get_lines |> List.filter is_valid |> List.length |> print_int;
  print_newline ();
  Util.get_lines input |> List.filter is_more_valid |> List.length |> print_int;
  print_newline ()
;;
