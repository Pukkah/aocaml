...
let get_root input =
  let names, mentioned =
    ...
    |> List.map (fun s ->
      let name, rest = Base.String.lsplit2_exn ~on:' ' s in
      let mentioned =
        if String.contains rest '>'
        then
          Base.String.rsplit2_exn ~on:'>' rest
          |> snd
    ...
    |> List.split
  in
  let names = SSet.of_list names
  and mentioned = List.flatten mentioned |> SSet.of_list in
  SSet.diff names mentioned |> SSet.to_list |> List.hd
;;

let run () = get_root input' |> print_endline
