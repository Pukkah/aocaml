    ...
    | [ r; a; v; _; cr; c; cv ] ->
      ...
      ; cond =
          (match c with
           ...
           | "<=" -> ( <= )
           | _ -> failwith "invalid input")
      ; cond_val = int_of_string cv
      }
    | _ -> failwith "invalid input")
;;

let compute instructions =
  let registers = Hashtbl.create 1000
  and max_val = ref 0 in
  let get_val reg = if Hashtbl.mem registers reg then Hashtbl.find registers reg else 0 in
  List.iter
    (fun { register; action; value; cond_reg; cond; cond_val } ->
      if cond (get_val cond_reg) cond_val
      then (
        let new_val = action (get_val register) value in
        Hashtbl.replace registers register new_val;
        max_val := max !max_val new_val))
    instructions;
  ( Hashtbl.to_seq_values registers
    |> List.of_seq
    |> List.sort ( - )
    |> List.rev
    |> List.hd
...
let run () =
  let max_end, max_running = parse input |> compute in
  print_int max_end;
  print_newline ();
  print_int max_running;
  print_newline ()
;;
