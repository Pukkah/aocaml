...
let group entry arr =
  let visited = ref ISet.empty
  and roadmap = ref (ISet.add_seq (List.to_seq arr.(entry)) ISet.empty) in
  while not (ISet.is_empty !roadmap) do
    let candidate = ISet.min_elt !roadmap in
    if ISet.mem candidate !visited
    then roadmap := ISet.remove candidate !roadmap
    else (
      visited := ISet.add candidate !visited;
      roadmap := ISet.add_seq (List.to_seq arr.(candidate)) !roadmap)
  done;
  !visited
;;

let count_groups _arr =
  let count = ref 0
  and remaining = ref (Seq.init (Array.length _arr) (fun i -> i) |> ISet.of_seq) in
  while not (ISet.is_empty !remaining) do
    let entry = ISet.min_elt !remaining in
    remaining := ISet.diff !remaining (group entry _arr);
    incr count
  done;
  !count
;;
...
let run () =
  parse input |> group 0 |> ISet.cardinal |> print_int |> print_newline;
  parse input |> count_groups |> print_int |> print_newline
;;
